...
#define APP_WIFI_SDIO_CCCR_IOABORT    0x06U
...
#define APP_WIFI_SDIO_CCCR_F2BLKSIZE_0 0x210U
#define APP_WIFI_SDIO_CCCR_F2BLKSIZE_1 0x211U
...
#define APP_WIFI_SDIO_FRAME_CONTROL   0x1000DU
...
#define APP_WIFI_SDIO_BACKPLANE_32BIT_ACCESS 0x08000U
...
#define APP_WIFI_SDIO_HT_TIMEOUT_MS   1000U
#define APP_WIFI_SDIO_FN2_ENABLE_TIMEOUT_MS 3000U
...
#define APP_WIFI_SOCRAM_WRAPPER_BASE   0x18104000U
...
#define APP_WIFI_SFC_RF_TERM    0x01U
...
__attribute__((section(".sdmmc_dma"), aligned(32)))
static uint32_t g_wifiCmd53Scratch[(APP_WIFI_STAGE_CHUNK_SIZE + sizeof(uint32_t) - 1U) / sizeof(uint32_t)];
...
HAL_StatusTypeDef APP_WiFi_Platform_ConfigureBus(void)
{
  ...
  if (APP_WiFi_Platform_Cmd52Write(APP_WIFI_SDIO_FN0, APP_WIFI_SDIO_CCCR_F2BLKSIZE_0, APP_WIFI_SDIO_BLOCK_SIZE) != HAL_OK)
  {
    return HAL_ERROR;
  }

  if (APP_WiFi_Platform_Cmd52Write(APP_WIFI_SDIO_FN0, APP_WIFI_SDIO_CCCR_F2BLKSIZE_1, 0U) != HAL_OK)
  {
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef APP_WiFi_Platform_Cmd53Read(uint8_t functionNumber, uint32_t address, uint8_t *data, uint16_t dataLength)
{
  ...
  APP_WiFi_Platform_DCacheClean((const void *)g_wifiCmd53Scratch, transferLength);
  ...
  SDMMC1->ARG = APP_WiFi_Platform_BuildCmd53Argument(0U,
                                                     functionNumber,
                                                     address,
                                                     0U,
                                                     1U,
  ...
  APP_WiFi_Platform_DCacheInvalidate((const void *)g_wifiCmd53Scratch, transferLength);
...
HAL_StatusTypeDef APP_WiFi_Platform_BackplaneRead(uint32_t address, uint8_t *data, uint16_t dataLength)
{
  ...
  if (dataLength == 1U)
  {
    status = APP_WiFi_Platform_Fn1Read8(windowOffset, data);
    (void)APP_WiFi_Platform_SetBackplaneWindow(APP_WIFI_CHIPCOMMON_BASE_ADDRESS);
    return status;
  }

  if (dataLength == sizeof(uint32_t))
  {
    windowOffset |= APP_WIFI_SDIO_BACKPLANE_32BIT_ACCESS;
  }

  status = APP_WiFi_Platform_Cmd53Read(APP_WIFI_SDIO_FN1, windowOffset, data, dataLength);
  (void)APP_WiFi_Platform_SetBackplaneWindow(APP_WIFI_CHIPCOMMON_BASE_ADDRESS);
...
HAL_StatusTypeDef APP_WiFi_Platform_BackplaneRead32(uint32_t address, uint32_t *value)
{
  ...
  {
    if (APP_WiFi_Platform_BackplaneRead(address + index, &data[index], 1U) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
...
HAL_StatusTypeDef APP_WiFi_Platform_BackplaneWrite(uint32_t address, const uint8_t *data, uint16_t dataLength)
{
  ...
  if (dataLength == 1U)
  {
    status = APP_WiFi_Platform_Fn1Write8(windowOffset, data[0]);
    (void)APP_WiFi_Platform_SetBackplaneWindow(APP_WIFI_CHIPCOMMON_BASE_ADDRESS);
    return status;
  }
  ...
  status = APP_WiFi_Platform_Cmd53Write(APP_WIFI_SDIO_FN1, windowOffset, data, dataLength);
  (void)APP_WiFi_Platform_SetBackplaneWindow(APP_WIFI_CHIPCOMMON_BASE_ADDRESS);
...
HAL_StatusTypeDef APP_WiFi_Platform_BackplaneWrite32(uint32_t address, uint32_t value)
{
  ...
  for (index = 0U; index < sizeof(data); ++index)
  {
    if (APP_WiFi_Platform_BackplaneWrite(address + index, &data[index], 1U) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
...
HAL_StatusTypeDef APP_WiFi_Platform_PrepareFirmwareDownload(void)
{
  ...
  if (APP_WiFi_Platform_DisableDeviceCore(APP_WIFI_SOCRAM_WRAPPER_BASE, 0U) != HAL_OK)
  {
    return HAL_ERROR;
  }

  if (APP_WiFi_Platform_ResetDeviceCore(APP_WIFI_SOCRAM_WRAPPER_BASE, 0U) != HAL_OK)
  {
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef APP_WiFi_Platform_GetFunction2RxInterruptStatus(uint32_t *frameStatus)
{
  ...
  g_wifiInterruptStatus = interruptStatus;
  *frameStatus = (interruptStatus & APP_WIFI_HOST_INT_MASK_VALUE);

  if ((*frameStatus != 0U) &&
      (APP_WiFi_Platform_BackplaneWrite32(APP_WIFI_SDIO_INT_STATUS_ADDRESS, *frameStatus) != HAL_OK))
  {
    return HAL_ERROR;
  }

  g_wifiInterruptStatus &= ~(*frameStatus);
...
HAL_StatusTypeDef APP_WiFi_Platform_ClearFunction2RxInterrupt(uint32_t frameStatus)
{
  ...
  if (APP_WiFi_Platform_BackplaneWrite32(APP_WIFI_SDIO_INT_STATUS_ADDRESS, frameStatus) != HAL_OK)
  {
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef APP_WiFi_Platform_AbortFunction2Read(void)
{
  if (APP_WiFi_Platform_Cmd52Write(APP_WIFI_SDIO_FN0,
                                   APP_WIFI_SDIO_CCCR_IOABORT,
                                   APP_WIFI_SDIO_FN2) != HAL_OK)
  {
    return HAL_ERROR;
  }

  if (APP_WiFi_Platform_Fn1Write8(APP_WIFI_SDIO_FRAME_CONTROL, APP_WIFI_SFC_RF_TERM) != HAL_OK)
  {
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef APP_WiFi_Platform_Fn2Read(uint8_t *data, uint16_t dataLength)
{
  ...
  if (dataLength >= APP_WIFI_SDIO_BLOCK_SIZE)
  {
    blockMode = 1U;
    count = (uint16_t)((dataLength + APP_WIFI_SDIO_BLOCK_SIZE - 1U) / APP_WIFI_SDIO_BLOCK_SIZE);
  }
  else
  {
    blockMode = 0U;
  ...
  memset((void *)g_wifiCmd53Scratch, 0, sizeof(g_wifiCmd53Scratch));
  APP_WiFi_Platform_DCacheClean((const void *)g_wifiCmd53Scratch, dataLength);
  ...
  APP_WiFi_Platform_DCacheInvalidate((const void *)g_wifiCmd53Scratch, dataLength);
  memcpy(data, (const void *)g_wifiCmd53Scratch, dataLength);
  return HAL_OK;
}
...
HAL_StatusTypeDef APP_WiFi_Platform_Fn2Write(const uint8_t *data, uint16_t dataLength)
{
  ...
  if (dataLength >= APP_WIFI_SDIO_BLOCK_SIZE)
  {
    transferLength = APP_WIFI_SDIO_BLOCK_SIZE;
    dmaTransferLength = (uint32_t)(((dataLength + APP_WIFI_SDIO_BLOCK_SIZE - 1U) / APP_WIFI_SDIO_BLOCK_SIZE) * APP_WIFI_SDIO_BLOCK_SIZE);
    dctrlBlockSize = SDMMC_DATABLOCK_SIZE_64B;
    blockMode = 1U;
    count = (uint16_t)(dmaTransferLength / APP_WIFI_SDIO_BLOCK_SIZE);
  }
  else
  {
  ...
  APP_WiFi_Platform_DCacheClean((const void *)g_wifiCmd53Scratch, dmaTransferLength);
  ...
  SDMMC1->ARG = APP_WiFi_Platform_BuildCmd53Argument(1U,
                                                     APP_WIFI_SDIO_FN2,
                                                     0U,
                                                     blockMode,
                                                     1U,
                                                                    ...
                                                                    uint32_t *bytesStaged)
{
  ...
  *bytesStaged = 0U;
    ...
    uint32_t sizeRead = 0U;
      ...
      if (chunkSize > APP_WIFI_SDIO_BLOCK_SIZE)
      {
        chunkSize = APP_WIFI_SDIO_BLOCK_SIZE;
      }
...
static void APP_WiFi_Platform_DCacheClean(const void *address, uint32_t length)
{
  ...
  start = ((uintptr_t)address) & ~(uintptr_t)31U;
  end = (((uintptr_t)address) + (uintptr_t)length + 31U) & ~(uintptr_t)31U;
